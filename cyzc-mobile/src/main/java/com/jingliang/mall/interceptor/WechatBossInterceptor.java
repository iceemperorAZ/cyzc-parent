...
@Slf4j
@Component
public class WechatBossInterceptor implements HandlerInterceptor {
    ...
    public WechatBossInterceptor(RedisService redisService) {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (Objects.isNull(user)) {
            ...
            response.setCharacterEncoding("UTF-8");
            response.setContentType("application/json;charset=UTF-8");
            response.getWriter().write(JSONObject.toJSONString(Result.build(MallConstant.AUTHORITY_FAIL, MallConstant.TEXT_AUTHORITY_FAIL)));
            log.debug("权限不足");
            return false;
        }
