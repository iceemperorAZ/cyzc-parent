...
public class UrlLogoutSuccessHandler implements LogoutSuccessHandler {
    ...
    public void onLogoutSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        if (StringUtils.isBlank(token)) {
            log.debug("用户注销失败");
            response.getWriter().write(JSONObject.toJSONString(Result.build(Msg.FAIL, Msg.TEXT_LOGOUT_FAIL)));
            return;
        }
        ...
        response.getWriter().write(JSONObject.toJSONString(Result.build(Msg.OK, Msg.TEXT_LOGOUT_OK)));
    }
}
