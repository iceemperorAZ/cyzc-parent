...
public interface OrderRepository extends BaseRepository<Order, Long> {
     ...
     * @return
     */
    @Query(value = "SELECT   o.id,b.order_specific_num AS orderSpecificNum, o.order_no AS orderNo, FORMAT( IFNULL(  o.total_price, 0 ) * 0.01 ,2) AS totalPrice, FORMAT( IFNULL(  o.payable_fee , 0 ) * 0.01 ,2) AS payableFee, FORMAT( IFNULL( o.preferential_fee , 0 ) * 0.01 ,2) AS preferentialFee, FORMAT( IFNULL(  o.total_price * o.ratio * 0.001 , 0 ) * 0.01  ,2)  AS royaltyPrice ,ANY_VALUE(o.return_gold) as returnGold ,ANY_VALUE(o.gold) as gold \n" +
            "FROM tb_order o JOIN  tb_buyer b ON o.buyer_id = b.id \n" +
     ...
     * @return
     */
    @Query(value = "SELECT g.group_no as groupNo,g.group_name as groupName,COUNT(1) as count FROM tb_buyer b JOIN tb_user u ON b.sale_user_id = u.id JOIN tb_group g ON g.group_no = u.group_no WHERE g.group_no like :groupNo AND DATE_FORMAT(b.create_time,'%Y-%m') = DATE_FORMAT(:date,'%Y-%m') GROUP BY g.group_no,g.group_name;\n", nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "SELECT g.group_no as groupNo,g.group_name as groupName,COUNT(1) as count FROM tb_buyer b JOIN tb_user u ON b.sale_user_id = u.id JOIN tb_group g ON g.group_no = u.group_no  WHERE g.group_no like :groupNo  GROUP BY g.group_no,g.group_name;", nativeQuery = true)
