...
public interface OrderRepository extends BaseRepository<Order, Long> {
    ...
    @Query(value = "SELECT g.id, g.group_no AS groupNo, g.group_name AS groupName, FORMAT(IFNULL( SUM( o.total_price ), 0 ) * 0.01,2) AS totalPrice, FORMAT(IFNULL( SUM( o.payable_fee ), 0 ) * 0.01,2) AS payableFee, FORMAT(IFNULL( SUM( o.preferential_fee ), 0 ) * 0.01,2) AS preferentialFee, FORMAT(IFNULL( SUM( o.total_price * o.ratio * 0.001 ), 0 ) * 0.01 ,2) AS royaltyPrice\n" +
            "FROM tb_group g LEFT JOIN tb_order o ON o.group_no LIKE CONCAT(regexp_replace(g.group_no ,'0*$',''), '%' )  AND o.order_status BETWEEN 300  AND 700  AND o.is_available = 1  AND g.is_available = 1  AND o.create_time BETWEEN :startTime AND :endTime \n" +
     ...
     */
    @Query(value = "SELECT g.id, g.group_no AS groupNo, g.group_name AS groupName,  FORMAT( IFNULL( SUM( o.total_price ), 0 ) * 0.01  ,2) AS totalPrice,  FORMAT( IFNULL( SUM( o.payable_fee ), 0 ) * 0.01 ,2) AS payableFee,  FORMAT( IFNULL( SUM( o.preferential_fee ), 0 ) * 0.01 ,2) AS preferentialFee,  FORMAT( IFNULL( SUM( o.total_price * o.ratio * 0.001 ), 0 ) * 0.01 ,2) AS royaltyPrice \n" +
            "FROM  tb_group g  LEFT JOIN tb_order o ON o.group_no LIKE CONCAT( REGEXP_REPLACE( g.group_no, '0*$', '' ), '%' )  AND o.is_available = 1  AND o.order_status BETWEEN 300   AND 700   AND o.create_time BETWEEN :startTime   AND :endTime \n" +
    ...
    @Query(value = "SELECT u.id, u.group_no AS groupNo, u.user_name AS userName, u.phone, FORMAT( IFNULL( SUM( o.total_price ), 0 ) * 0.01 ,2) AS totalPrice, FORMAT( IFNULL( SUM( o.payable_fee ), 0 ) * 0.01,2) AS payableFee, FORMAT( IFNULL( SUM( o.preferential_fee ), 0 ) * 0.01,2) AS preferentialFee, FORMAT( IFNULL( SUM( o.total_price * o.ratio * 0.001 ), 0 ) * 0.01,2)  AS royaltyPrice FROM tb_user u LEFT JOIN tb_order o " +
            "ON u.id = o.sale_user_id AND o.group_no LIKE :groupNo AND o.order_status BETWEEN 300 AND 700 AND o.is_available = 1 AND u.is_available = 1 AND o.create_time BETWEEN :startTime AND :endTime  WHERE u.group_no like :groupNo  GROUP BY u.group_no, u.id, u.user_name, u.phone ORDER BY u.group_no ASC", nativeQuery = true)
    ...
    @Query(value = "SELECT b.id, b.user_name AS userName, b.phone, b.shop_name AS shopName, FORMAT( IFNULL( SUM( o.total_price ), 0 ) * 0.01 ,2) AS totalPrice, FORMAT( IFNULL( SUM( o.payable_fee ), 0 ) * 0.01 ,2) AS payableFee, FORMAT( IFNULL( SUM( o.preferential_fee ), 0 ) * 0.01 ,2) AS preferentialFee, FORMAT( IFNULL( SUM( o.total_price * o.ratio * 0.001 ), 0 ) * 0.01  ,2)  AS royaltyPrice \n" +
            "FROM tb_buyer b LEFT JOIN tb_order o ON b.id = o.buyer_id  AND o.group_no LIKE :groupNo AND o.order_status BETWEEN 300  AND 700  AND o.is_available = 1  AND b.is_available = 1 AND o.create_time BETWEEN :startTime AND :endTime \n" +
            "WHERE o.sale_user_id=:saleUserId \n" +
    ...
    @Query(value = "SELECT b.id, b.user_name AS userName, b.shop_name AS shopName, b.phone, FORMAT( IFNULL( SUM( o.total_price ), 0 ) * 0.01 ,2) AS totalPrice, FORMAT( IFNULL( SUM( o.payable_fee ), 0 ) * 0.01 ,2) AS payableFee, FORMAT( IFNULL( SUM( o.preferential_fee ), 0 ) * 0.01 ,2) AS preferentialFee, FORMAT( IFNULL( SUM( o.total_price * o.ratio * 0.001 ), 0 ) * 0.01  ,2)  AS royaltyPrice \n" +
            "FROM tb_buyer b LEFT JOIN tb_order o ON b.id = o.buyer_id   AND o.order_status BETWEEN 300  AND 700  AND o.is_available = 1  AND b.is_available = 1  AND o.create_time BETWEEN :startTime AND :endTime \n" +
            "WHERE o.sale_user_id=:saleUserId \n" +
    ...
    @Query(value = "SELECT " +
            ...
            "FROM tb_order_detail AS o_d  JOIN tb_order o ON o_d.order_id = o.id JOIN tb_group g ON o.group_no LIKE :groupNo JOIN tb_product p ON o_d.product_id = p.id JOIN tb_product_type p_t ON p.product_type_id = p_t.id WHERE o.order_status BETWEEN 300 AND 700 AND o.create_time BETWEEN  :startTime AND :endTime AND o.is_available = 1 AND o_d.is_available = 1 AND  p.is_available = 1 AND p_t.is_available = 1 GROUP BY p_t.product_type_name;\n", nativeQuery = true)
    ...
    @Query(value = "SELECT  p.product_name AS productName, FORMAT( SUM( IFNULL(  o_d.selling_price * o_d.product_num, 0 )) * 0.01,2) AS totalPrice, FORMAT( SUM( IFNULL(  o_d.selling_price * o_d.product_num, 0 )) * 0.01 ,2) AS payableFee, 0 AS preferentialFee, FORMAT( SUM(IFNULL(  o_d.selling_price * o_d.product_num * o.ratio * 0.001 , 0 )) * 0.01  ,2)  AS royaltyPrice \n" +
            "FROM tb_order_detail AS o_d  JOIN tb_order o ON o_d.order_id = o.id JOIN tb_group g ON o.group_no LIKE :groupNo  JOIN tb_product p ON o_d.product_id = p.id  WHERE o.order_status BETWEEN 300 AND 700 AND o.create_time BETWEEN :startTime AND :endTime AND  o.is_available = 1 AND o_d.is_available = 1 AND  p.is_available = 1 GROUP BY p.product_name", nativeQuery = true)
